...
class IOSScreenBase < Calabash::IBase
  ...
  def enter_password(password)
    touch("* id:'#{eletronic_password_access_buton}'")
  ...
  def set_field(element_alias, text, index = -1)
    #begin
      ...
      if idx >= 0
        ...
        all_elements = get_all_elements("* id:'#{get_element_id(element_alias)}'")
      ...
      when TYPE[:Selector]
        ...
        uia_query = 'uia.selectPickerValues(\'{0 "' + text + '"}\')'
  ...
  def check_element_is_enabled(element_alias, expected_enabled)
    begin
      drag_until_element_is_visible(:down, get_element(element_alias))
      actual_enabled = query("* id:'#{get_element(element_alias)}'").first["enabled"]
    rescue
